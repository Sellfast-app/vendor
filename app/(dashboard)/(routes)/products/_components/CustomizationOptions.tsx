...
export default function CustomizationOptions() {
    ...
    // Draft form state — one group form at a time
    ...
    // Committed groups shown in the summary list below
    ...
    const handleAddGroup = () => {
        ...
        if (existing) {
            // Append option to existing group
            setSavedGroups((prev) =>
                prev.map((g) =>
                    g.groupName === groupName
                        ? {
                            ...
                            options: [
                                ...g.options,
                                { id: Math.random().toString(36).substr(2, 9), name: optionName, price: optionPrice },
                            ],
                        }
                        : g
                )
        ...
        // Reset only the option fields, keep group context so user can add more options
        setOptionName('');
        setOptionPrice('');
    };
                    ...
                    <div className="border border-dashed border-[#4FCA6A] rounded-lg p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Select
                                    value={selection}
                                    onValueChange={(v) => setSelection(v as 'single' | 'multiple')}
                                >
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
                        ...
                        <div className="flex flex-col gap-1.5">
                            <Label className="text-xs font-light">
                                Is this required?<span className="text-destructive">*</span>
                            </Label>
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <Label htmlFor="customization-required" className="text-xs font-medium cursor-pointer">
                            ...
                            {savedGroups.map((group) => (
                                <div key={group.id}>
                                    ...
                                    {group.options.map((opt) => (
                                        <div
                                            key={opt.id}
                                            className="flex items-center justify-between py-2 border-b border-[#F0F0F0] last:border-0 pl-1"
                                        >
                                            ...
                                            <div className="flex items-center gap-3">
                                                <span className="text-sm font-medium flex items-center gap-0.5">
                                                    ₦
                                                    <span className="ml-1">
                                                        {parseFloat(opt.price || '0').toLocaleString('en-NG', { minimumFractionDigits: 2 })}
                                                    </span>
                                                </span>
                                                <button
                                                    type="button"
