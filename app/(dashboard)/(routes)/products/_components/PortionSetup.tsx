...
import { PortionSize } from './AddFoodModal';

interface PortionSetupProps {
    portions: PortionSize[];
    onChange: (portions: PortionSize[]) => void;
}
...
export default function PortionSetup({ portions, onChange }: PortionSetupProps) {
    ...
    const resetForm = () => {
        setPortionName(''); setPortionPrice(''); setPrepTimeStart('');
        setPrepTimeEnd(''); setServings(''); setServingUnit('plate');
    };
    ...
    const handleAdd = () => {
        ...
        onChange([...portions, {
            ...
            prepTimeStart, prepTimeEnd, servings, servingUnit,
        }]);
        resetForm();
    };
    ...
    const handleEdit = (id: string) => {
        ...
        if (!p) return;
        setPortionName(p.name); setPortionPrice(p.price);
        setPrepTimeStart(p.prepTimeStart); setPrepTimeEnd(p.prepTimeEnd);
        setServings(p.servings); setServingUnit(p.servingUnit);
        onChange(portions.filter((p) => p.id !== id));
    };

    const handleRemove = (id: string) => onChange(portions.filter((p) => p.id !== id));
            ...
            {isOpen && (
                <div className="mt-3 space-y-3">
                    <div className="border border-dashed border-[#4FCA6A] rounded-lg p-4 space-y-3">
                        ...
                        <div>
                            <Label className="text-xs font-light mb-1">Portion Price<span className="text-destructive">*</span></Label>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <Label className="text-xs font-light mb-1">Prep Time Start<span className="text-destructive">*</span></Label>
                                <div className="flex items-center border rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-ring">
                                    <span className="px-2 text-xs text-[#A0A0A0] select-none whitespace-nowrap">Mins</span>
                                    <input type="number" min={0} value={prepTimeStart} onChange={(e) => setPrepTimeStart(e.target.value)} placeholder="0" className="flex-1 py-2 pr-3 text-sm outline-none bg-transparent" />
                                </div>
                            </div>
                            <div>
                                <Label className="text-xs font-light mb-1">Prep Time Finish<span className="text-destructive">*</span></Label>
                                <div className="flex items-center border rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-ring">
                                    <span className="px-2 text-xs text-[#A0A0A0] select-none whitespace-nowrap">Mins</span>
                                    <input type="number" min={0} value={prepTimeEnd} onChange={(e) => setPrepTimeEnd(e.target.value)} placeholder="0" className="flex-1 py-2 pr-3 text-sm outline-none bg-transparent" />
                                </div>
                            </div>
                        </div>
                        <div>
                            <Label className="text-xs font-light mb-1">
                                Serves <span className="text-[#A0A0A0]">(How many servings does this portion contain?)</span><span className="text-destructive">*</span>
                            </Label>
                            <div className="flex items-center border rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-ring">
                                ...
                                <input type="number" min={0} value={servings} onChange={(e) => setServings(e.target.value)} placeholder="0" className="flex-1 py-2 pr-3 text-sm outline-none bg-transparent" />
                            </div>
                        </div>
                    </div>

                    <Button type="button" variant="outline" size="sm" className="text-xs flex items-center gap-1.5" onClick={handleAdd}>
                        <CirclePlus className="w-3.5 h-3.5" /> Add Portion Size
                    </Button>
