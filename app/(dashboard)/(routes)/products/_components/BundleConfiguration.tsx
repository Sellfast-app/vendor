...
interface AddOnGroup {
    ...
    isRequired: boolean;
    options: AddOnOption[];
    // transient UI state — the input fields before "Add Option" is clicked
...
function AddOnGroupCard({
    ...
    group: AddOnGroup;
    onChange: (id: string, field: keyof AddOnGroup, value: unknown) => void;
    onRemove: (id: string) => void;
    ...
    return (
        <div className="border border-dashed border-[#4FCA6A] rounded-lg p-4 mb-4">
            <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => onRemove(group.id)} type="button" className="text-[#A0A0A0] hover:text-red-500 transition-colors">
                    <X className="w-4 h-4" />
                </button>
            </div>
            ...
            <div className="mb-3">
                ...
                <Input
                    ...
                    onChange={(e) => onChange(group.id, 'groupName', e.target.value)}
                    placeholder="e.g. Proteins, Drinks, Carbs"
                />
            </div>

            {/* Selection + Max Selections */}
            <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                    ...
                    <Select
                        ...
                        onValueChange={(v) => onChange(group.id, 'selection', v as 'single' | 'multiple')}
                    >
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
                ...
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => onChange(group.id, 'maxSelections', parseInt(e.target.value) || 1)}
                    />
                </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5 mb-4">
                ...
                <div className="flex items-center gap-2">
                    <input
                        ...
                        onChange={(e) => onChange(group.id, 'isRequired', e.target.checked)}
                        className="w-3.5 h-3.5 accent-[#4FCA6A] cursor-pointer"
                    />
                    <Label htmlFor={`required-${group.id}`} className="text-xs font-medium cursor-pointer">
                        Yes, customers must select
                    </Label>
                </div>
            </div>
            ...
            <div className="border-t border-[#F0F0F0] pt-3">
                ...
                <div className="mb-3">
                    ...
                    <Input
                        value={group.draftOptionName}
                        onChange={(e) => onChange(group.id, 'draftOptionName', e.target.value)}
                        placeholder="e.g. Extra Chicken"
                        className="mb-2"
                    />
                    <Label className="text-xs font-light mb-1">
                        Add-on Price<span className="text-destructive">*</span>
                    </Label>
                    <div className="flex items-center gap-1 border rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-ring">
                        ...
                        <input
                            ...
                            min={0}
                            step="0.01"
                            value={group.draftOptionPrice}
                            onChange={(e) => onChange(group.id, 'draftOptionPrice', e.target.value)}
                            placeholder="0.00"
...
export default function BundleConfiguration() {
    // Bundle slots
    ...
    const handleEditSlot = (id: string) => {
        ...
        if (!slot) return;
        setBundleName(slot.name);
        setBundlePrice(slot.price);
        setBundleCategory(slot.category);
        setMaxItems(slot.maxItems);
        setIsSlotRequired(slot.isRequired);
        setSlots((prev) => prev.filter((s) => s.id !== id));
    };
    ...
    const handleGroupChange = (id: string, field: keyof AddOnGroup, value: unknown) => {
        setAddOnGroups((prev) =>
    ...
    const handleAddOption = (groupId: string) => {
        setAddOnGroups((prev) =>
            prev.map((g) => {
                ...
                if (!g.draftOptionName.trim()) return g;
                return {
                    ...g,
                    options: [
                        ...g.options,
                        {
                            id: Math.random().toString(36).substr(2, 9),
                            name: g.draftOptionName,
                            price: g.draftOptionPrice,
                        },
                    ],
                    draftOptionName: '',
                    draftOptionPrice: '',
                ...
                {bundleOpen && (
                    <div className="pb-4">
                        ...
                        <div className="border border-dashed border-[#4FCA6A] rounded-lg p-4 mt-4 space-y-3">
                            ...
                            <div>
                                <Label className="text-xs font-light mb-1">
                                    Bundle Price<span className="text-destructive">*</span>
                                </Label>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <Select value={bundleCategory} onValueChange={setBundleCategory}>
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="fruity-desserts">Fruity Deserts</SelectItem>
                                ...
                                <div>
                                    <Label className="text-xs font-light mb-1">
                                        Max Items Allowed<span className="text-destructive">*</span>
                                    </Label>
                                    <Input
                                        type="number"
                                        min={1}
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        type="checkbox"
                                        id="slot-required"
                                        checked={isSlotRequired}
                                        onChange={(e) => setIsSlotRequired(e.target.checked)}
                                        className="w-3.5 h-3.5 accent-[#4FCA6A] cursor-pointer"
                                    />
                                    <Label htmlFor="slot-required" className="text-xs font-medium cursor-pointer">
                                        Yes, customers must select
                                    </Label>
                                </div>
                            </div>
            ...
            <div className=" overflow-hidden">
                <div className="flex items-start justify-between">
                    ...
                    <Switch
                        ...
                        className="data-[state=checked]:bg-[#4FCA6A] mt-0.5"
                    />
                </div>

                {customizationEnabled && (
                    <div className=" pb-4">
                        {/* Active add-on group cards */}
                        <div className="mt-4">
